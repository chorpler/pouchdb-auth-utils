...
var Headers = PouchFetch.Headers;
...
var rdb1 = new PouchDB(dbname);
var rdb2 = new TestPouch(dbname);

var url1 = "../_membership";
var headObject = {"Content-Type": "application/json", "Accept": "application/json"};
var hdr1 = new Headers(headObject)
var opts = {method:"GET", headers: hdr1};
async function tryFetch () {
  try {
    console.log("tryFetch() called ...");
    // let res = await rdb1.fetch(url1, opts);
    let res = await rdb2.fetch(url1, opts);
...
function doit() {
    ...
    async function loop() {
        while(g && !g.quitIt) {
           console.log("TRYING ITERATION " + counter);
           try {
               let res = await tryFetch();
               console.log("FINISHED ITERATION " + counter + " SUCCESSFULLY");
               console.log("DONE WITH ITERATION " + counter++);
           } catch(err) {
                console.log("ERROR WITH ITERATION " + counter);
                console.error(err);
                console.log("DONE WITH ITERATION " + counter++);
           }
        }
